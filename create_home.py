...
class _ScanHomeScreenState extends ConsumerState<ScanHomeScreen> {
  ...
  void _openScan() async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => const QuestionShareScreen()));
    _loadCredits();
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: CustomScrollView(slivers: [
          ...
          // CONTENT
          if (_isLoading)
            const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.only(top: 100), child: Center(child: CircularProgressIndicator(color: Color(0xFF6C5CE7)))))
          else if (!hasScans)
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Mekan\\u0131 Tara', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14)),
      ),
    );
  }
        ...
        GestureDetector(
          ...
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [const Color(0xFF6C5CE7).withOpacity(0.08), const Color(0xFF8B5CF6).withOpacity(0.05)]),
              borderRadius: BorderRadius.circular(99), border: Border.all(color: const Color(0xFF6C5CE7).withOpacity(0.12))),
  ...
  Widget _buildEmptyState() {
    return Padding(
      ...
      child: Column(children: [
        // Koala hero
        Image.asset('assets/images/koala_hero.png', height: 120),
        ...
        Text('Mekan\\u0131n\\u0131n foto\\u011fraf\\u0131n\\u0131 \\u00e7ek,\\nKoala stilini analiz etsin', textAlign: TextAlign.center,
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(width: 48, height: 48, decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: Colors.white.withOpacity(0.2)),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('Foto\\u011fraf \\u00e7ek, saniyeler i\\u00e7inde analiz al', style: TextStyle(fontSize: 13, color: Colors.white.withOpacity(0.7))),
              ])),
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          _SubjectPill(emoji: '\\ud83c\\udfe0', label: 'Salon', color: const Color(0xFF6C5CE7)),
          _SubjectPill(emoji: '\\ud83c\\udf73', label: 'Mutfak', color: const Color(0xFFEC4899)),
          _SubjectPill(emoji: '\\ud83d\\udecf', label: 'Yatak Odas\\u0131', color: const Color(0xFF00B894)),
          _SubjectPill(emoji: '\\ud83d\\udebf', label: 'Banyo', color: const Color(0xFF38BDF8)),
          _SubjectPill(emoji: '\\ud83d\\udcbc', label: 'Ofis', color: const Color(0xFFF59E0B)),
        ]),
        const SizedBox(height: 28),
        // Nasil calisir
        Align(alignment: Alignment.centerLeft,
          child: Text('NASIL \\u00c7ALI\\u015eIR', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Colors.grey.shade400, letterSpacing: 1))),
        const SizedBox(height: 14),
        Row(children: [
          _HowCard(icon: Icons.camera_alt_rounded, color: const Color(0xFF6C5CE7), title: '\\u00c7ek', desc: 'Mekan\\u0131n foto\\u011fraf\\u0131n\\u0131 \\u00e7ek'),
          const SizedBox(width: 10),
          _HowCard(icon: Icons.auto_awesome, color: const Color(0xFFEC4899), title: 'Analiz', desc: 'Koala stilini analiz etsin'),
          const SizedBox(width: 10),
          _HowCard(icon: Icons.explore_rounded, color: const Color(0xFF00B894), title: 'Ke\\u015ffet', desc: 'evlumba\\'da ke\\u015ffet'),
        ]),
      ]),
    );
  }
...
class _SubjectPill extends StatelessWidget {
  const _SubjectPill({required this.emoji, required this.label, required this.color});
  final String emoji; final String label; final Color color;
  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 9),
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: color.withOpacity(0.06), border: Border.all(color: color.withOpacity(0.1))),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
...
class _HowCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(child: Container(
      ...
      child: Column(children: [
        Container(width: 36, height: 36, decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), color: color.withOpacity(0.08)),
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Text(scan.analysis?.detectedStyle ?? (analyzing ? 'Analiz ediliyor...' : 'Hata'), 
                style: TextStyle(fontSize: 14, fontWeight: isNew ? FontWeight.w700 : FontWeight.w600,
