...
class QuestionStore extends ChangeNotifier {
  ...
  // ── SORTING: last_interacted_at > created_at (en yeni etkileşim en üstte) ──
  void _sortQuestions() {
    _questions.sort((a, b) {
      final aTime = a.lastInteractedAt ?? a.createdAt;
      final bTime = b.lastInteractedAt ?? b.createdAt;
      ...
      for (final row in rows) {
        ...
        final q = LocalQuestion(
          ...
          lastInteractedAt: row['last_interacted_at'] != null
              ? DateTime.tryParse(row['last_interacted_at'] as String)
              : null,
        );
  ...
  void markRead(String id) {
    final q = getById(id);
    if (q == null || q.isRead) return;
    q.isRead = true;
    notifyListeners();
    _updateFieldInDb(id, {'is_read': true});
  }
  ...
  void toggleFavorite(String id) {
    ...
    notifyListeners();
    _updateFieldInDb(id, {'is_favorite': q.isFavorite});
  }

  // ── UPDATE LAST INTERACTED (soru en üste çıkar) ──
  void touchInteraction(String id) {
    final q = getById(id);
    if (q == null) return;
    q.lastInteractedAt = DateTime.now();
    _sortQuestions();
    notifyListeners();
    _updateFieldInDb(id, {'last_interacted_at': DateTime.now().toIso8601String()});
  }

  // ── GENERIC DB FIELD UPDATE ──
  Future<void> _updateFieldInDb(String id, Map<String, dynamic> fields) async {
    if (_uid == null) return;
    try {
      await _sb.from('questions').update(fields).eq('id', id);
    } catch (e) {
  ...
  void addChat(String id, ChatMsg msg) {
    ...
    if (q == null) return;
    q.chatMessages = [...q.chatMessages, msg];
    // Her mesajda etkileşim zamanını güncelle
    q.lastInteractedAt = DateTime.now();
    _sortQuestions();
    notifyListeners();
